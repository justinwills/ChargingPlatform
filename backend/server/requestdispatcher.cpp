...
{
    const QString phoneKeyword = params.value("phoneKeyword").toString().trimmed();
    QJsonArray users;
    for (const UserInfo &user : Database::getAllUsers(phoneKeyword)) {
        QJsonObject item;
        item["userId"] = user.id;
...
QJsonObject RequestDispatcher::handleSetUserStatus(const QJsonObject &params)
{
    ...
    if (userId <= 0 || (status != QStringLiteral("正常")
                        && status != QStringLiteral("冻结"))) {
        return fail(1, "userId或status参数无效");
    }

    if (!Database::setUserStatus(userId, status)) {
...
QJsonObject RequestDispatcher::handleAdminQueryStations(const QJsonObject &)
{
    ...
    for (const StationInfo &station : Database::getAllStations()) {
        ...
        item["pileCount"] = station.pileCount;
        item["freePileCount"] = Database::getFreePileCount(station.id);
        item["onlineRate"] = Database::getStationOnlineRate(station.id);
...
QJsonObject RequestDispatcher::handleAdminQueryPiles(const QJsonObject &)
{
    ...
    for (const PileInfo &pile : Database::getAllPiles()) {
        QJsonObject item;
        item["pileId"] = pile.id;
        item["stationId"] = pile.stationId;
        item["stationName"] = pile.stationName;
        item["code"] = pile.code;
        item["type"] = pile.type;
