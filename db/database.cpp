...
static bool insertIdlePiles(QSqlDatabase &db, int stationId, int startIndex, int count)
{
    for (int i = 0; i < count; ++i) {
        ...
        pileQuery.prepare("insert into piles(station_id, code, type, power, status) "
                          "values(?, ?, '快充', 60, '闲置')");
        pileQuery.addBindValue(stationId);
        pileQuery.addBindValue(QStringLiteral("S%1-%2")
                               .arg(stationId)
                               .arg(startIndex + i, 2, 10, QChar('0')));
        if (!pileQuery.exec()) {
            qDebug() << "insertIdlePiles 失败：" << pileQuery.lastError().text();
            return false;
        }
    }
    return true;
